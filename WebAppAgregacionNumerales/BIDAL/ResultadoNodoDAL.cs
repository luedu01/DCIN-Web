            ...
            using (TdConnection oSqlConnection = new TdConnection(Cnn))
            {
                try
                {
                    ...
                    using (TdCommand oSqlCmd = new TdCommand())
                    {
                        ...
                        oSqlCmd.CommandText = "SEL NOD.Sk_NodoContable Sk_NodoContable, NOD.Desc_NodoContable, NOD.Id_NodoContable, COALESCE(PAD.Id_NodoContable, 0) Id_NodoContablePadre, PAD.Desc_NodoContable Desc_NodoContablePadre ,  COALESCE(AGR.Sk_RCNumeralCambiario, 0) Sk_RCNumeralCambiario,   NOD.Num_Nivel, 0 idnumeralcco FROM " + @instancia + ".V_RC_EstructuraAgregacionNumerales EST"
                            ...
                            + " where EST.Id_Estructura = ? and EST.Desc_Estructura = ? and EST.Cb_Eliminado <> 'S' and NOD.Cb_eliminado <> 'S';";
